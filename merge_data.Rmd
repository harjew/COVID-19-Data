...
get_covid_germany <- function(level = 1){
  ...
  if(level== 1){
    ...
    i <- 1
    for(d in date){
      dfm <- sum(ge2_death$No_death[which(ge2_death$gender == "male" & ge2_death$date == d)])
      dff <- sum(ge2_death$No_death[which(ge2_death$gender == "female" & ge2_death$date == d)])
      dfnc <- sum(ge2_death$No_death[which(ge2_death$gender == "NC" & ge2_death$date == d)])
      dft <- dfnc + dff + dfm
      ...
      a_it <- 1
       for(a in age_groups){
        ...
        df <- rbind(df,
                     ...
                     Year = y,
                     WeekNo = w,
                     male_covid_death = sum(df2$No_death[df2$gender == "male"]), 
                     female_covid_death = sum(df2$No_death[df2$gender == "female"]), 
                     nc_covid_death = sum(df2$No_death[df2$gender == "NC"]),
                     total_covid_death = sum(df2$No_death), 
                     country = "Germany", 
                     type = "AgeGender"
                    ))
        a_it <- a_it + 1
...
```{r}
sweden1 <- get_covid_sweden(level = 1)
      ...
      a_it <- 1
       for(a in age_groups){
        ...
        df <- rbind(df,
                     ...
                     Year = y,
                     WeekNo = w,
                     male_covid_death = sum(df2$deaths[df2$sex == "male"]), 
                     female_covid_death = sum(df2$deaths[df2$sex == "female"]), 
                     nc_covid_death = sum(df2$deaths[df2$sex == "NC"]),
                     total_covid_death = sum(df2$deaths), 
                     country = "Spain", 
                     type = "AgeGender"
                    ))
        a_it <- a_it + 1
...
get_covid_belgium <- function(level = 1){
  ...
  if(level== 1){
    ...
    i <- 1
    for(d in date){
      dfm <- sum(belgium$DEATHS[which(belgium$SEX == "M" & belgium$DATE == d)])
      dff <- sum(belgium$DEATHS[which(belgium$SEX == "female" & belgium$DATE == d)])
      dfnc <- sum(belgium$DEATHS[which(is.na(belgium$SEX) & belgium$DATE == d)])
      dft <- dfnc + dff + dfm
      w <- strftime(d, format = "%V")
      y <- strsplit(d,"-")[[1]][1]
      df[i,] <- c(d,y,w, dft,dfm, dff, dfnc)
      ...
      a_it <- 1
       for(a in age_groups){
        ...
        df <- rbind(df,
                     ...
                     Year = y,
                     WeekNo = w,
                     male_covid_death = sum(df2$DEATHS[df2$sex == "M"]), 
                     female_covid_death = sum(df2$DEATHS[df2$sex == "F"]), 
                     nc_covid_death = sum(df2$DEATHS[is.na(df2$SEX)]),
                     total_covid_death = sum(df2$DEATHS), 
                     country = "Belgium", 
                     type = "AgeGender"
                    ))
...
switzerland2 <- get_covid_switzerland(level = 2)
