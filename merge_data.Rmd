...
get_covid_uk <- function(level = 1){
  ...
  if(level == 1 | level == 4){
    ...
    df <- data.frame(WeekNo = c(as.integer(data[1,3:(nc-1)])),
                     ...
                     date = date,
                     total_covid_deaths = total_covid_death,
                     male_covid_deaths = male_covid_death,
                     female_covid_deaths = female_covid_death,
                     country = "UK",
                     Type = "Gender"
                     )
    l$level1 <- df
...
write.csv2(germany3, "covid-data/COVID-19-GERMANY-level-3.csv")
